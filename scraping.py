...
def download_images(images, folder_name):
    ...
    if len(images) != 0:
        for i, image_link in enumerate(images):
            try:
                urllib.request.urlretrieve(image_link, 'letter_{}.jpg'.format(adjusted_alphabet[i]))
                img = Image.open('letter_{}'.format(adjusted_alphabet[i]))
                img.save(r'C:/Users/jkorr/Downloads/main/images')
                count += 1
            except:
                pass
